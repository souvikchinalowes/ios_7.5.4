...
- (void)doCopyMove:(CDVInvokedUrlCommand*)command isCopy:(BOOL)bCopy
{
    ...
    //[Ronit S.] Start - 30.0.11 - Liberty change of filename
    // NSString* newName = ([arguments count] > 2) ? [arguments objectAtIndex:2] : [srcURL.url lastPathComponent];
    
    NSDate* nsToday = [NSDate date];
    NSDateFormatter* dateFormat  = [[NSDateFormatter alloc] init];
    [dateFormat setDateFormat:@"_hhmmss."];
    NSString* dateString = [dateFormat stringFromDate:nsToday];
    
    //NSString* newName =[dateString stringByAppendingString: ([arguments count] > 2) ? [arguments objectAtIndex:2] : [srcURL.url lastPathComponent]];
    NSString* origName = ([arguments count] > 2) ? [arguments objectAtIndex:2] : [srcURL.url lastPathComponent];
    //NSString *origName = @"AB.CD.EF.jpg";
    NSRange dotLoc = [origName rangeOfString:@"."  options:NSBackwardsSearch];
    NSString* newName;
    if( dotLoc.location != NSNotFound)
    newName = [origName stringByReplacingCharactersInRange:dotLoc withString:dateString ];
    else
    newName= origName;
