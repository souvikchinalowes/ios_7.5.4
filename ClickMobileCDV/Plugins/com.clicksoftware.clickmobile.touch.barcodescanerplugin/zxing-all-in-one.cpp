...
namespace zxing {
    ...
    Ref<Result> MultiFormatReader::decode(Ref<BinaryBitmap> image, const char* formats /*Rotem - barcode format*/) {
        setHints(DecodeHints::DEFAULT_HINT, formats /*Rotem - barcode format*/);
    ...
    Ref<Result> MultiFormatReader::decode(Ref<BinaryBitmap> image, DecodeHints hints, const char* formats /*Rotem - barcode format*/) {
        setHints(hints, formats /*Rotem - barcode format*/);
        return decodeInternal(image);
    }
    
    Ref<Result> MultiFormatReader::decodeWithState(Ref<BinaryBitmap> image, const char* formats /*Rotem - barcode format*/) {
    ...
    void MultiFormatReader::setHints(DecodeHints hints, const char* formats /*Rotem - barcode format*/) {
        hints_ = hints;
        readers_.clear();
        ...
        if (addOneDReader && !tryHarder) {
            readers_.push_back(Ref<Reader>(new zxing::oned::MultiFormatOneDReader(hints, formats /*Rotem - barcode format*/)));
        }
        
        // Rotem - barcode format
        if (formats != NULL && formats[0] != '\0') {
            char QRCode[] = "QRCode";
            char DataMatrix[] = "DataMatrix";
            
            if (strstr(formats, QRCode)) {
                readers_.push_back(Ref<Reader>(new zxing::qrcode::QRCodeReader()));
            }
            if (strstr(formats, DataMatrix)) {
...
namespace {
    size_t wordsForSize(size_t width,
        ...
        //convert long to int - change the code
        long arraySize =
            (bits + bitsPerWord - 1)
                >>
            logBits;
...
namespace zxing {
    ...
    int BitSource::available() {
                    //          idan ofek - convert to float
        return 8 *
            (int)(bytes_.size() - byteOffset_)
                - bitOffset_;
    }
}
...
namespace zxing {
    namespace EdgeDetector {
        ...
        Line findLine(const BitMatrix& image, Line estimate, bool invert, int deviation, float threshold, int skip) {
            ...
            //idan ofek - added convartion to int
...
namespace zxing {
    ...
    int GlobalHistogramBinarizer::estimate(vector<int> &histogram) {
        //idan ofek - convert to int implicit
...
//          idan ofek - convert to int
    ...
    void GridSampler::checkAndNudgePoints(Ref<BitMatrix> image, vector<float> &points) {
        ...
        //idan ofek
        //unsigned int width/height change
...
//      idan ofek - convert to int directliy
...
namespace zxing {
    ...
    Ref<GF256Poly> GF256Poly::addOrSubtract(Ref<GF256Poly> b) {
        ...
        //          idan ofek - convert to int
        unsigned lengthDiff =(int) (largerCoefficients.size() - smallerCoefficients.size());
...
namespace zxing {
    namespace oned {
        MultiFormatOneDReader::MultiFormatOneDReader(DecodeHints hints, const char* formats /*Rotem - barcode format*/) : readers() {
            
            // Rotem - barcode format
            if (formats != NULL && formats[0] != '\0') {
                char MultiFormatUPCEAN[] = "MultiFormatUPCEAN";
                char Code39[] = "Code39";
                char Code128[] = "Code128";
                char ITF[] = "ITF";
                
                if (strstr(formats, MultiFormatUPCEAN)) {
                    readers.push_back(Ref<OneDReader>(new MultiFormatUPCEANReader(hints)));
                }
                if (strstr(formats, Code39)) {
                    readers.push_back(Ref<OneDReader>(new Code39Reader()));
                }
                if (strstr(formats, Code128)) {
                    readers.push_back(Ref<OneDReader>(new Code128Reader()));
                }
                if (strstr(formats, ITF)) {
