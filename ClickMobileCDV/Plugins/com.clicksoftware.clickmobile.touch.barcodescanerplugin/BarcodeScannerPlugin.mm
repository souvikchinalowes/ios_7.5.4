...
// plugin class
//------------------------------------------------------------------------------
@interface BarcodeScanerPlugin : CDVPlugin {}
- (NSString*)isScanNotPossible;
- (void)scanningBarcode:(CDVInvokedUrlCommand*)command;
...
@property (nonatomic, retain) BarcodeScanerPlugin*           plugin;
...
@property (nonatomic,retain)         AVCaptureDevice* __block    device;

- (id)initWithPlugin:(BarcodeScanerPlugin*)plugin callback:(NSString*)callback parentViewController:(UIViewController*)parentViewController alterateOverlayXib:(NSString *)alternateXib;
- (void)scanBarcode;
...
// plugin class
//------------------------------------------------------------------------------
@implementation BarcodeScanerPlugin
...
//--------------------------------------------------------------------------
- (id)initWithPlugin:(BarcodeScanerPlugin*)plugin
...
//    [self.viewController.view addGestureRecognizer:singleFingerTap];
...
//    UITouch *touch = obj;
    CGPoint touchPoint = [recognizer locationInView:self.viewController.view];
    self.previewLayer.frame = CGRectMake((touchPoint.x-25), (touchPoint.y-25), 50, 50);
...
//            [_device setExposurePointOfInterest:CGPointMake(focus_x,focus_y)];
...
//--------------------------------------------------------------------------
- (void)barcodeScanSucceeded:(NSString*)text format:(NSString*)format {
    // Rotem - Fix for not-working cancel and flip camera buttons
...
- (NSString*)setUpCaptureSession {
    ...
    //can remove because this varible is systenzed
...
//    AVCaptureDevice* __block device = nil;
    if (self.isFrontCamera) {
        ...
        if (!_device) return @"unable to obtain video capture device";
//        else
//        {
...
////                [device setFocusPointOfInterest:CGPointMake(screenWidthHalf, screenHeightHalf)];
...
//        }
    ...
    if([input.device isFocusPointOfInterestSupported] && [input.device isFocusModeSupported:AVCaptureFocusModeAutoFocus]){
        
        NSError *error = nil;
        [input.device lockForConfiguration:&error];
        input.device.focusMode = AVCaptureFocusModeAutoFocus;
        [input.device setFocusMode:AVCaptureFocusModeAutoFocus];
//                        [device setFocusPointOfInterest:CGPointMake(screenWidthHalf, screenHeightHalf)];
...
//  Idan Ofek - bug 75304 - Barcode scanner is not working on ios - resubmit the code
//    dispatch_queue_t qBarCode = dispatch_queue_create("cam.bar.code.queue", NULL);
//    [output setSampleBufferDelegate:self queue:qBarCode];
//    [output setSampleBufferDelegate:self queue:dispatch_get_main_queue()]; - Rotem - removed
//  end - bug 75304 - Barcode scanner is not working on ios
...
//    [self.viewController.view.window addSubview:flashView];
    ...
    try {
        ...
        // Rotem - barcode format
        NSString *path = [[NSBundle mainBundle] pathForResource:@"ClickMobileCDV-Info" ofType:@"plist"];
        NSMutableDictionary *dict = [[NSMutableDictionary alloc] initWithContentsOfFile:path];
        NSString *formats = [dict objectForKey:@"LimitBarcodeScanner"];
        const char* formats_char = [formats cStringUsingEncoding:NSUTF8StringEncoding];
        ...
        Ref<Result>            result            (reader->decode(bitmap, decodeHints, formats_char)); // Rotem - barcode format
...
//    if (
//        [previewLayer isOrientationSupported]
//        ) {
//        
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if ([self.processor.previewLayer respondsToSelector:@selector(connection)]) {
        
        self.processor.previewLayer.connection.videoOrientation
...
- (UIView*)buildOverlayView {
    ...
    UIBarButtonItem *flexSpace = [[[UIBarButtonItem alloc] autorelease]
                    initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace
                    target:nil
                    action:nil
                    ];
    
    UIBarButtonItem *flipCamera = [[[UIBarButtonItem alloc] autorelease]
...
#if USE_SHUTTER
    UIButton *shutterButton = [[UIBarButtonItem alloc]
                        initWithBarButtonSystemItem:UIBarButtonSystemItemCamera
                        target:(id)self
